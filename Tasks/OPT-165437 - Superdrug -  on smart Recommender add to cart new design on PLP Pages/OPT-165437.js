...
Insider.__external.SetLoader165437 = (variationId) => {
    ((self) => {
        ...
        self.handleAddToCartButtonClick = () => {
            ...
            const $buttons = Insider.dom(wrapper).find(addToCart);

            Insider.eventManager.once(`${ clickMethod }.add:to:cart:button:click:${ variationId }`, $buttons, (e) => {
                const $target = Insider.dom(e.target);
                const $targetButton = Insider.dom(e.target).find(addToCartText).length ?
