...
(function () {
    if (!Insider.__external.isObservedDefinedSRTR12594) {
        var pageSelectors = '.page-homepage, .page-productDetails, .pageType-CategoryPage, .page-cartPage';
        ...
        var observer = new MutationObserver(function (mutations) {
            mutations.forEach(function () {
                var initTypeControl = Insider.__external.oldUrlSRTR12594 !== window.location.href;
                ...
                if (initTypeControl) {
                    ...
                    setTimeout(function () {
                        var cartSelectors = '.cart-page__product-summary:visible, e2-is-visible .cart-page__empty-text:visible';
                        var productSelectors = '.pdp__notify-button:visible, [v-slot="in-stock"]:visible';
...
Insider.__external.changeLocationConfig = function (options) {
    ...
    function changeLocation(variationId) {
        var camp = Insider.campaign.get(variationId) || {};
        var ps = camp.pageSettings || camp.ps || {};
        var locationConfig = ps.locationConfig || {};
...
Insider.__external.ajaxListener(function (url, response, method) {
    if (url.indexOf('https://recommendationv2.api.useinsider.com/') > -1 && method === 'GET') { /* OPT-127374 */
        var recommendedProducts = (JSON.parse(response || '{}') || {}).data || [];
        var recommendedProductIds = recommendedProducts.map(function (product) {
            return product.item_id;
        });
        var desiredRecommenderSelector = '';
        ...
        setTimeout(function () {
            Insider.fns.onElementLoaded(desiredRecommenderSelector, function () {
                var eventLabel = 'category grid page';
                ...
                setTimeout(() => { /* OPT-133065 */
                    Insider.dom(desiredRecommenderSelector).find('[ins-product-id]').nodes.forEach(function (element) {
                        var $addToCartElement = Insider.dom(element);
                        var productId = $addToCartElement.attr('ins-product-id');
                        var productInformation = recommendedProducts.filter(function (product) {
                            return product.item_id === productId;
                        })[0] || {};

                        if (Insider.fns.isObject(productInformation.product_attributes)) {
                            var isMultipleVariation = Number(productInformation.product_attributes.variation_count) > 1;
                            var partnerAddToCartHTML = '';
