...
((self) => {
    ...
    const category = Insider.systemRules.call('getCategories')[0] ?? '';
    ...
    self.init = () => {
        if (variationId) {
            ...
            if (isOnCategoryPage || isOnProductPage || Insider.systemRules.getSearchKeyWords()) { /* OPT-157602 */
                self.setVisitedTime();
    ...
    self.setVisitedTime = () => {
        /* OPT-157602 */
        const isSearchCategory = !!Insider.systemRules.getSearchKeyWords();
        const categoryName = isSearchCategory ? category.match(/'([^']+)'/)?.[1] ?? '' : category;

        if (Insider.fns.has(categoryNames, categoryName)) {
            /* OPT-157602 */
            const visitedCategory = `ins-visited-${ categoryName }-${ variationId }`;
    ...
    self.setLastVisitedCategories = () => {
        ...
        if (Insider.fns.has(categoryNames, categoryName)) {
            ...
            const categoryIndex = lastVisitedCategories.indexOf(categoryName);

            if (categoryIndex > -1) {
                lastVisitedCategories.splice(categoryIndex, 1);
            }

            lastVisitedCategories.unshift(categoryName);
