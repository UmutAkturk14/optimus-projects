...
const tca = 'total-cart-amount';
const insT = 'ins-temp-product-list';
const clc = 'click.insClick';
const aC = 'ins-after-payment-refresh-control';
const t = Insider.dateHelper.now();
        ...
        if (isOldListContainsNewProduct) {
            ...
            if (upP.id) {
                if (upP.quantity > nP.quantity) {
                    nP.quantity = upP.quantity - nP.quantity;
                    dispatchEvent(nP, false);

                } else {
                    nP.quantity = nP.quantity - upP.quantity;
...
function setStorage(totalQty, totalAmount, productL) {
    ...
    productL = Insider.utils.opt.mergeDuplicateProducts(productL);

    Insider.utils.cart.storeCartProductStorage({ totalQuantity: totalQty, productList: productL });
    sSet('paid-products', productL);
    sSet('total-cart-amount', parseFloat(totalAmount.toFixed(2)) || 0);

    /* OPT-163473 START */
    salesLog.track('set-storage', {
        oldProductList,
        productList: productL,
        paidProducts: Insider.systemRules.call('getPaidProducts'),
        totalCartAmount: Insider.systemRules.call('getTotalCartAmount'),
    });
    /* OPT-163473 END */
    dT();
    c(productL, oldProductList);

}

function s(p) {
    const ps = Insider.storageAccessor.cartProductList();
    let tA = Insider.storageAccessor.totalCartAmount();
    const pL = ps.productList || [];
    let tQ = ps.totalQuantity || 0;

    pL.push(p);

    tA += p.price * p.quantity;
...
function cUp() {
    setTimeout(() => {
        ...
        dT();
        c(Insider.systemRules.call('getPaidProducts'), oldPList);
...
function fetchListener(callback) {
    ...
    window.fetch = function () {
        const ftch = orjFetch.apply(this, arguments);
        let url = '';
...
if (!(Insider.__external.isApiInit23911 || false)) {
    Insider.__external.isApiIni23911 = true;
    Insider.utils.opt.ajaxListener(function (url) {
        if (url.indexOf('updateitemassembly') && Insider.fns.hasParameter('checkout')) {
            setTimeout(() => {
                sSet(tca, parseFloat((pP(Insider.dom('.cart-total .order-total:first').text()) - pP(Insider.dom('.cart-total-right .product-price:eq(1)').text())).toFixed(2)) || Insider.storageAccessor.totalCartAmount() || 0);
    ...
    Insider.eventManager.on(clc, '.pip-btn[data-test-target=add-to-cart-button], .pip-buy-module__buttons', function () {
        ...
        if (Insider.systemRules.call('isOnProductPage')) {
            ...
            if (q > 0 && Insider.dom('.pip-upsell').exists()) {
                const el = Insider.dom('.pip-upsell .pip-product-compact');
                const prc = pP(Insider.dom(el).attr('data-price'));

                if (prc > 0 && cP.price > 0) {
                    const url = Insider.dom('a[class*=wrapper-link]', el).prop('href') || '';
                    const p = { id: urlSplit(url), name: encodeURIComponent((Insider.dom(el).attr('data-product-name') || '').trim()), price: prc, originalPrice: pP(Insider.dom('[class*=price--secon] .pip-price__integer', el).text()) || prc, img: (Insider.dom('[class*=image-wrap] img', el).prop('src') || '').split('?')[0], url, cats: [], quantity: q, time: t };
    ...
    Insider.eventManager.on(clc, '[class*=pip-product-compact__], .button__add-to-cart', function () {
        Insider.__external.pAddSrtr18394 = false;
        const el = window.location.pathname.indexOf('search/visual-search') !== -1 ? Insider.dom(this).closest('.search-grid__item.product-fragment') : Insider.dom(this).closest('.pip-product-compact');
        const prc = pP(window.getComputedStyle((Insider.dom('.pip-price', el).nodes[0] || document.createElement('div')), ':after').getPropertyValue('content')) || pP(Insider.dom(el).attr('data-price')) || pP(Insider.dom('.price-package__price:first', el).text()) || pP(Insider.dom('.pip-temp-price__nowrap:first', el).text());

        if (prc > 0) {
            const url = Insider.dom('a', el).prop('href') || '';
            const p = { id: urlSplit(url), name: encodeURIComponent((Insider.dom(el).attr('data-product-name') || '').trim()), price: prc, originalPrice: pP(Insider.dom('[class*=through] .pip-price__integer:first', el).text()) || prc, img: (Insider.dom('[class*=image__image], img', el).attr('srcset') || '').trim().split(',')[0].split(' ')[0].split('?')[0], url, cats: [], quantity: 1, time: t };
    ...
    Insider.eventManager.on(clc, '.plp-btn.plp-btn--small', function () {
        Insider.__external.pAddSrtr18394 = false;
        const el = Insider.dom(this).closest('.plp-fragment-wrapper');
        const prc = pP(Insider.dom('.plp-price__nowrap:first', el).text());

        if (prc > 0) {
            const url = Insider.dom('a', el).prop('href') || '';
            const p = { id: urlSplit(url), name: encodeURIComponent(Insider.dom('.notranslate:first', el).text().trim()), price: prc, originalPrice: prc, img: (Insider.dom('img', el).prop('src') || '').split('?')[0], url, cats: [], quantity: 1, time: t };
    ...
    if (window.location.pathname.indexOf('favourites') !== -1) {
        Insider.eventManager.on(clc, '[class*=addToCartWrapper] [class*=prefix-btn__inner]', function () {
            Insider.__external.pAddSrtr18394 = false;
            const el = Insider.dom(this).closest('section[class*=RecommendationItem]');
            const qty = parseInt(Insider.dom('input[class*=quantity]', el).val()) || 1;
            const prc = parseFloat((pP(Insider.dom('[class*=price-module__current-price] span:first', el).text()) / qty).toFixed(2)) || 0;

            if (prc > 0) {
                const url = Insider.dom('a', el).prop('href') || '';
                const p = { id: urlSplit(url), name: encodeURIComponent(Insider.dom('[class*=productTitle]', el).text().trim()), price: prc, originalPrice: prc, img: Insider.dom('[class*=fix-image]', el).attr('src') || '', url, cats: [], quantity: qty, time: t };
        ...
        Insider.eventManager.on(clc, '[class*=ListProduct_main] [data-testid=add-to-bag-button]', function () {
            Insider.__external.pAddSrtr18394 = false;
            const el = Insider.dom(this).closest('section[class*=ListProduct_product]');
            const qty = parseInt(Insider.dom('input[class*=quantity]', el).val()) || 1;
            const prc = parseFloat((pP(Insider.dom('[class*=price-module__current-price] span:first', el).text()) / qty).toFixed(2)) || 0;

            if (prc > 0) {
                const url = Insider.dom('a', el).prop('href') || '';
                const p = { id: urlSplit(url), name: encodeURIComponent(Insider.dom('[class*=productTitle]', el).clone().children().remove().end().text().trim()), price: prc, originalPrice: prc, img: Insider.dom('[class*=fix-image]', el).attr('src') || '', url, cats: [], quantity: qty, time: t };
        ...
        Insider.eventManager.on(clc, '[class*=Summary_addToCartButton]', function () {
            Insider.__external.pAddSrtr18394 = false;
            Insider.dom('[class*=ListProduct_product]').accessNodes(function (e) {
                const qty = parseInt(Insider.dom('input[class*=quantity]', e).val()) || 1;
                const prc = parseFloat((pP(Insider.dom('[class*=fix-price__integer]', e).text()) / qty).toFixed(2)) || 0;

                if (prc > 0) {
                    const url = Insider.dom('a', e).prop('href') || '';
                    const p = { id: urlSplit(url), name: encodeURIComponent(Insider.dom('[class*=productTitle]', e).text().trim()), price: prc, originalPrice: prc, img: Insider.dom('[class*=fix-image]', e).attr('src') || '', url, cats: [], quantity: qty, time: t };
...
if (Insider.systemRules.call('isOnCartPage')) {
    Insider.__external.isRefSrtr18394 = true;
    cUp();
