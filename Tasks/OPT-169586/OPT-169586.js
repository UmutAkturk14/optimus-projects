...
((self) => {
    ...
    const selectors = Insider.fns.keys(classes).reduce((createdSelector, key) => (
        ...
        badgeText: `span:contains("${ currentBadgeText }"),
            .m-cargoTag__text:contains("${ currentBadgeText }"),
            .m-basket__fastDelivery:contains("${ currentBadgeText }")`,
    ...
    self.reset = () => {
        const { style, join: joinSelector, newBadgeText: newBadgeTextSelector } = selectors;
        const { join } = classes;
    ...
    self.addClass = () => {
        const { discountedProduct } = selectors;
        const { join } = classes;
