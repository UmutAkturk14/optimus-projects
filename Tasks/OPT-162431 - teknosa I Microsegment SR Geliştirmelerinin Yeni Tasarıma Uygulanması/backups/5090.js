...
/* OPT-145245 START Microsegment1 codes */
const userId = Insider.getUserId();
const segmentId = ((Insider.storage.localStorage.get('ins-mikro-segment-id-145245') || {})[userId] || {})
    .firstMicroSegmentId || '0';
const purchasedProductCategories = [];

Insider.storageAccessor.userPurchased().forEach((product) => {
    const categoryIndex = product.cats.length > 2 ? 2 : 0;

    purchasedProductCategories.push(product.cats[categoryIndex] || '');
});
/* OPT-145245 END Microsegment1 Codes */
...
const templateConfig = {
    ...
    hooks: {
        fillGeneralProductContent: {
            after(product) {
                /* OPT-159543 START */
                const { productNameAttribute, customReviewWrapper, starElementSelector, imageBoxSelector, priceSelector,
                    originalPriceSelector, discount, ratingAttributeSelector,
                ...
                if (reviewCountAttribute && rating) {
                    ...
                    $templateContainer.find(imageBoxSelector).css('margin-bottom', '24');
                }
                ...
                if (price === originalPrice) {
                    $templateContainer.find(priceSelector).css('padding', '0px 8px 8px 0px');
                    ...
                    $templateContainer.find(priceSelector).css('padding', '0px 8px 8px 8px');
                    $templateContainer.find(discount).css('width', '100%', 'important');
                    $templateContainer.find(originalPriceSelector).css('display', 'flex', 'important');
                }

                $templateContainer.find(productNameAttribute).css('padding', '0px 0px 0px 0px');
                $templateContainer.find(originalPriceSelector).css('padding', '8px 8px 0px 8px');
      ...
      /* OPT-145245 START microsegment1 codes */
        getEndpoint: {
            self() {
                return `${ this.getEndpoint() }&filter=([product_attributes.micro_segment_rank1][~][${ segmentId }:]*[category][!=][${ purchasedProductCategories.join('||') || '-' }])`;
            }
        },
        generateTemplate: {
            self(endpoint) {
                ...
                Insider.request.get({
                    ...
                    success: function (response) {
                        ...
                        const getMicroSegmentRank = product => ((product.product_attributes || {})
                            .micro_segment_rank1 || []).find(rank => rank.split(':')[0].includes(`${ segmentId }`)); /* OPT-161581 */
                        ...
                        Insider.request.get({
                            ...
                            success: function (response) {
                                ...
                                /* const newData = sortedData.sort((acculmator, currentValue) =>
                            sortKey(acculmator) - sortKey(currentValue)); */
...
const sliderInit = function (callback) {
    ...
    smartRecommenderSlider.setDimensions = function () {
        ...
        Insider.dom(elements.productImageBox).height(eachItemWidth * 1.2); /* OPT-159543 */
