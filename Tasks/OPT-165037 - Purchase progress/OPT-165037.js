...
Insider.__external.PurchaseProgressionCampaigns164169 = (variationId) => {
    ((self) => {
        ...
        self.init = () => {
            if (variationId) {
                if ((Insider.systemRules.call('isOnProductPage') || Insider.systemRules.call('isOnCategoryPage'))) {
                    if (!isControlGroup) {
                        self.reset();
                        self.buildCSS();
                    }
        ...
        self.checkCampaignVisibility = () => {
            ...
            /* OPT-165037 START */
            if (Insider.storage.localStorage.get(storageName) && self.getBasketAmount() > 30) {
