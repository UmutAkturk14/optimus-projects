...
((self) => {
    ...
    self.addClass = () => {
        const { partnerSizeWrapper } = selectors;
        const { overflowVisible } = classes;
    ...
    /* OPT-150501 START */
    self.setGoals = () => {
        const { partnerSizeContainer } = selectors;
        const { specificSize, relative, goal } = classes;
        ...
        Insider.dom(partnerSizeContainer).accessNodes((element) => {
            if (productSpecificSizes.some((size) => size.toLowerCase() === element.textContent.toLowerCase())) {
                Insider.dom(element).find('span').addClass(`${ specificSize } ${ relative } ${ goal }`);
            }
        });
    };
    /* OPT-150501 END */
