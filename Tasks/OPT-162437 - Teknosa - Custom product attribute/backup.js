...
var templateConfig = {
    ...
    hooks: {
        ...
        getEndpoint: {
            self: function () {
                return `${ this.getEndpoint() }&filter=([product_attributes.micro_segment_id2][~][${ segmentId }:]*[category][!=][${ purchasedProductCategories.join('||') || '-' }])`;
            }
        },
        generateTemplate: {
            self(endpoint) {
                ...
                Insider.request.get({
                    ...
                    success: function (response) {
                        const { data } = response;
                        /* OPT-159417 START */
                        const sortBySegmentId = (firstProduct, secondProduct) => {
                            const getSegmentIndex = (segmentArray, id) => {
                                const matchingSegment = segmentArray.find((segment) => segment.startsWith(id));

                                return matchingSegment ? parseInt(matchingSegment.split(':')[1]) : -1;
                            };

                            const indexA = getSegmentIndex(firstProduct.product_attributes.micro_segment_rank2,
                                `${ segmentId }:`);
                            const indexB = getSegmentIndex(secondProduct.product_attributes.micro_segment_rank2,
                                `${ segmentId }:`);
                        ...
                        const sortedData = data.sort(sortBySegmentId);
                        /* OPT-159417 END */
