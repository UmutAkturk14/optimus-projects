...
((self) => {
    ...
    const bannerTitle = 'Sezonun Favori Modelleri';
    ...
    const classes = {
        ...
        campaignTitle: `ins-custom-campaign-title-${ variationId }`
    };
    ...
    self.reset = () => {
        const { style, wrapper, campaignTitle } = selectors;

        Insider.dom(`${ style }, ${ wrapper }, ${ campaignTitle }`).remove();
    };

    self.buildCSS = () => {
        const { wrapper, image, container, sideContainer, imageWrapper, campaignTitle } = selectors;
    ...
    self.buildHTML = () => {
        const { wrapper, container, image, sideContainer, imageWrapper, join, campaignTitle } = classes;
        const { appendLocation } = selectors;

        const html =
        `<p class="${ campaignTitle }">${ bannerTitle }</p>
        <div class="${ wrapper }">
            <div class="${ container }">
