...
Insider.eventManager.once(`focusin.set:searchbar:focus:${ camp.id }`, document, (event) => {
    const { dynamicInput, searchBarInput } = templateConfig.ELEMENTS;
...
self.buildCSS = () => {
    const { searchBarInputWrapper, searchBarInput } = templateConfig.ELEMENTS;
...
((self) => {
    ...
    const classes = {
        ...
        searchBarWrapperStyle: `ins-search-bar-wrapper-${ camp.id }`,
        searchBarInputStyle: `ins-search-bar-input-${ camp.id }`
    };
    ...
    self.init = () => {
        if (!Insider.campaign.isControlGroup(camp.id)) {
            self.reset();
            self.buildCSS();
            self.addClass();
            self.setEvents();
    ...
    self.reset = () => {
        const { searchBarInputWrapperStyle, searchBarInputStyle } = classes;
        const { searchBarInputWrapper, searchBarInput, style } = selectors;

        Insider.dom(style).remove();
        Insider.dom(searchBarInputWrapper).removeClass(searchBarInputWrapperStyle);
        Insider.dom(searchBarInput).removeClass(searchBarInputStyle);
    };

    self.buildCSS = () => {
        const { searchBarWrapper, searchBarInput } = selectors;
    ...
    self.addClass = () => {
        ...
        Insider.dom(searchBarInputWrapper).addClass(searchBarInputWrapperStyle);
        Insider.dom(searchBarInput).addClass(searchBarInputStyle);
    };
