...
((self) => {
    ...
    self.init = () => {
        if (variationId && self.checkEligibility()) { /* OPT-160188 */
    ...
    self.checkEligibility = () => {
        ...
        Insider.fns.onElementLoaded(categoryPageProductBox, () => {
            Insider.dom(categoryPageProductBox).nodes.forEach((node) => {
    ...
    self.buildCSS = () => {
      ...
      ${ wrapper } {
          width: 100%;
          ${ isCategoryPage ? 'margin-top: -10px;' : 'margin-bottom: 10px;' }
      }`;
    ...
    const handlePopState = () => {
        Insider.logger.log('Popstate event triggered.');
        Insider.logger.log('Rerunning the code...');
    ...
    Insider.eventManager.once('popstate.run:code:again', window, handlePopState);
