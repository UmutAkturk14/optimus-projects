...
((self) => {
    ...
    self.init = () => {
        const campaignStatus = !Insider.storage.session.get(`ins-memory-game-done-${ memoryGameVariationId }`)
            && !hasClosedCampaign;

        if (variationId) { /* OPT-167483 */
    ...
    self.setEvents = () => {
        ...
        const memoryGameCloseButton = `.ins-close-button-${ memoryGameVariationId }`;
        /* OPT-164287 END */
        ...
        Insider.fns.onElementLoaded(memoryGameCloseButton, () => {
            Insider.eventManager.once(`${ eventName }.close:button:${ variationId }`, memoryGameCloseButton, () => {
                Insider.utils.opt.sendCustomGoal(builderId, closeGoalId, true);
            });
        }).listen();
    };
